...
class HomeViewModel: ObservableObject {
    @Published var marketModel: MarketModel? = nil
    ...
    init() {
        $showShareBottomSheet
            ...
            .sink { [weak self] value in
                ...
                if value {
                    if let marketModel, let doInfo = marketModel.doInfo, let expireTime = doInfo.expireTime {
                        ...
                        let leftButton = Button().then {
                            ...
                            $0.tapAction = {
                                ...
                                DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) { [weak self] in
                                    guard let self, let marketModel else { return }
