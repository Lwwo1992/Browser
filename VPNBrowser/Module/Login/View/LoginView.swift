...
struct LoginView: View {
    ...
    private func LoginTypeView() -> some View {
        VStack(spacing: 20) {
            if mailbox.isEmpty {
                // 显示手机号码输入框
                if let phoneType = L.config.loginType?.first(where: { $0.key == "mobile" }), phoneType.value == true {
                    HStack {
                        Text("+86")
                            .font(.system(size: 14))
            ...
            if mobile.isEmpty {
                // 显示邮箱输入框
                if let emailType = L.config.loginType?.first(where: { $0.key == "mailbox" }), emailType.value == true {
                    HStack {
            ...
            // 判断是否展示 "或"
            if let accountType = L.config.loginType?.first(where: { $0.key == "account" }), accountType.value == true,
               (L.config.loginType?.contains(where: { $0.value == true })) ?? false {
                Button {
                    nextButtonAction()
                } label: {
                    Text("下一步")
    ...
    private func bottomView() -> some View {
        HStack {
            ...
            agreementButton(title: "《隐私协议》", requestData: 3, titleText: "隐私协议")
        }
        .font(.system(size: 12))
    }
...
extension LoginView {
    private func fetchAgreementContent(requestData: Int, titleText: String) {
        HUD.showLoading()
        APIProvider.shared.request(.rankingPage(data: requestData), model: ConfigByTypeModel.self) { result in
            HUD.hideNow()
            switch result {
